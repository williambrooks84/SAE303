...
function aggregateMonthlyRentalsByGenre(rentalsData) {
  const result = [];
  const genres = [...new Set(rentalsData.map((rental) => rental.genre))];
  const months = [...new Set(rentalsData.map((rental) => rental.month))];

  months.forEach((month) => {
    genres.forEach((genre) => {
      const rental = rentalsData.find((rental) => rental.month === month && rental.genre === genre);
      result.push({
        date: new Date(`${month}-01`).getTime(), // Convert month to timestamp
        genre: genre,
        value: rental ? parseFloat(rental.total_rentals) : 0, // Set value to 0 if no rentals
      });
    });
  });
...
let RentalsEvolutionGenreView = {
  render: function (containerId, rentalsData, moviesData) {
    ...
    const aggregatedData = aggregateMonthlyRentalsByGenre(rentalsData, moviesData);
    ...
    const label = root.container.children.push(
      am5.Label.new(root, {
      text: "Evolution des locations selon le genre sur les 6 derniers mois (en €)", // Set the desired static label text
      x: 0,
      y: -10,
      fontSize: 16,
...
export { RentalsEvolutionGenreView };
