...
function aggregateSalesByCountry(SalesData) {
  return SalesData.map((sale) => ({
    category: sale.country,
    value: parseFloat(sale.total_sales),
  }));
}

let SalesByCountryView = {
  render: function (containerId, SalesData) {
    ...
    // Set data
    const aggregatedData = aggregateSalesByCountry(SalesData);
    ...
    const label = root.container.children.push(
      am5.Label.new(root, {
      text: "Volume des gains des ventes par pays (en €)", // Set the desired static label text
      x: am5.percent(50),
      centerX: am5.percent(50),
      y: -10,
      fontSize: 16,
